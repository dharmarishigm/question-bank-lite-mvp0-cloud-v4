...
window.setupSharedGoogleEnrollment=async(token,exam)=>{
  ...
  async function signedIn(user){
    ...
    form.innerHTML='<label>Google account<input name="email" type="email" readonly></label><label>Mobile number (optional)<input name="phone_number" type="tel" autocomplete="tel" placeholder="+91 98765 43210" minlength="10" maxlength="20"></label><p class="registration-hint">Use your own mobile number. A number can belong to only one account.</p><button class="primary" type="submit">Enroll in this exam</button>';
    form.elements.email.value=user.email;
    try{const profile=await api('/api/student/profile');form.elements.phone_number.value=profile.phone_number||'';}catch{}
    message.textContent='Your Google email is verified. You may enroll without a mobile number.';
    form.onsubmit=async e=>{e.preventDefault();const button=form.querySelector('button');button.disabled=true;message.textContent='Enrolling and opening your exam…';try{const enrolled=await api(`/api/register/exam/${encodeURIComponent(token)}/enroll`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone_number:form.elements.phone_number.value})});$('public-registration').hidden=true;history.replaceState({},'','/app');window.sharedRegistrationSignedIn=null;await finishLogin(current);if(exam.proctor_required){showView('available-exams');await loadAvailableExams();notify('You are enrolled. Enter the proctor code to start this exam.');}else{const session=await api(`/api/exams/${enrolled.exam_id||exam.id}/sessions`,{method:'POST'});await openSecureExam(session.session_id);notify('Your exam is ready.');}}catch(error){message.textContent=error.message;}finally{button.disabled=false;}};
  }
