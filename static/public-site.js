(() => {
  ...
  fetch('/api/public/commerce-config',{cache:'no-store'}).then(r=>r.ok?r.json():null).then(config=>{if(!config?.demo_book?.enabled||!config.demo_book.url)return;const grid=document.getElementById('public-program-list');if(!grid)return;const card=document.createElement('article');card.className='program-product-card demo-book-card';card.innerHTML=`<span class="program-badge">FREE DEMO</span><h3>${esc(config.demo_book.title)}</h3><p>Preview an approved MeritIQra sample before registering.</p><a class="public-cta" href="${esc(config.demo_book.url)}" target="_blank" rel="noopener">View demo book</a>`;grid.prepend(card);}).catch(()=>{});
