(() => {
  ...
  fetch('/api/public/exams',{cache:'no-store'}).then(r=>r.ok?r.json():[]).then(rows=>{
    ...
    target.innerHTML=rows.map(e=>'<article class="public-exam-card"><span class="public-eyebrow">'+esc(e.exam_type||'ASSESSMENT')+'</span><h3>'+esc(e.name)+'</h3><p>'+esc(e.description||'A structured MeritIQra assessment.')+'</p><div class="exam-card-meta"><span>'+esc(e.subject||'General')+'</span><span>'+esc(e.level||'All levels')+'</span><span>'+Number(e.question_count||0)+' questions</span><span>'+Number(e.duration_minutes||0)+' min</span><span>'+(e.proctor_required?'Proctored':'Practice')+'</span></div>'+(e.allow_self_registration?'<button class="public-cta" data-enroll-exam="'+Number(e.id)+'">Register or Login</button>':'<button class="public-cta" data-student-login>Student Login</button>')+'</article>').join('');
