(() => {
  ...
  const openRegistration=examId=>{closeOverlays();registration.hidden=false;document.body.classList.add('public-login-open');const select=document.getElementById('public-registration-exam');if(examId)select.value=String(examId);document.querySelector('#student-registration-form input')?.focus();};
  ...
  document.getElementById('student-registration-form')?.addEventListener('submit',async event=>{
    event.preventDefault();const form=event.currentTarget,data=Object.fromEntries(new FormData(form));const status=document.getElementById('student-registration-message'),button=form.querySelector('[type=submit]');button.disabled=true;status.textContent='Registering and enrolling…';
    try{const examId=Number(data.exam_id);delete data.exam_id;const response=await fetch('/api/public/exams/'+examId+'/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});const body=await response.json().catch(()=>({}));if(!response.ok)throw new Error(body.detail||'Registration failed');location.assign('/');}
  ...
  fetch('/api/public/exams',{cache:'no-store'}).then(r=>r.ok?r.json():[]).then(rows=>{
    ...
    target.innerHTML=rows.map(e=>'<article class="public-exam-card"><span class="public-eyebrow">'+esc(e.exam_type||'ASSESSMENT')+'</span><h3>'+esc(e.name)+'</h3><p>'+esc(e.description||'A structured MeritIQra assessment.')+'</p><div class="exam-card-meta"><span>'+esc(e.subject||'General')+'</span><span>'+esc(e.level||'All levels')+'</span><span>'+Number(e.question_count||0)+' questions</span><span>'+Number(e.duration_minutes||0)+' min</span><span>'+(e.proctor_required?'Proctored':'Practice')+'</span></div>'+(e.allow_self_registration?'<button class="public-cta" data-enroll-exam="'+Number(e.id)+'">Register / Enroll</button>':'<button class="public-cta" data-student-login>Student Login</button>')+'</article>').join('');
