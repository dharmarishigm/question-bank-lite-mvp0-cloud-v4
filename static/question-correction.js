...
window.QuestionCorrection=(()=>{
  ...
  async function open(q,{save,onSaved,programPaperId}={}){
    ...
    modal.innerHTML=`<h3>${q.id?'Correct question #'+Number(q.id):'Correct review question'}</h3><p>Edit wording, equations, options, answer and solution. AI suggestions require your review.</p><p class="correction-progress" role="status" aria-live="polite"></p><form><label>Question<textarea name="statement" rows="5" required></textarea></label><div data-options></div><div class="actions"><button type="button" data-add-option>Add option</button><button type="button" data-remove-option>Remove last option</button></div><label>Correct answer (option letter or value)<input name="answer" /></label><label>Solution<textarea name="solution" rows="5"></textarea></label><label>Instructions for AI<textarea name="instructions" rows="2" placeholder="Describe the transcription, equation or answer issue; or specify how to regenerate."></textarea></label><div class="actions"><button class="ai-correct-action" type="button" data-ai="correct">Correct with AI</button><button class="ai-regenerate-action" type="button" data-ai="regenerate">Regenerate with AI</button>${programPaperId?'<button class="ai-replace-action" type="button" data-ai="replace_from_paper">Replace from paper syllabus</button>':''}<button type="button" data-preview-correction>Preview changes</button></div><div data-proposal hidden><h4>AI suggestion — review before applying</h4><div data-ai-notes></div><div class="rendered" data-ai-preview></div><button type="button" data-apply-proposal>Use suggestion in editor</button><button type="button" data-dismiss-proposal>Discard suggestion</button></div><div class="rendered" data-correction-preview></div><label class="inline"><input type="checkbox" name="reviewed" required />I reviewed the rendered question, options, answer and solution.</label><p class="muted">${save?'Updates this review draft. Save/publish it through its normal review workflow.':'Updates the question bank, paper previews and exam review. Active exams show the corrected question; changed wording or options require students to review their response. Submitted attempts retain their recorded response and score.'}</p><div class="actions"><button class="primary" type="submit">${save?'Update review question':'Update question'}</button><button type="button" data-close-correction>Cancel</button></div></form>`;
    ...
    const form=modal.querySelector('form'),status=modal.querySelector('[role=status]'),options=form.querySelector('[data-options]');
    ...
    form.querySelectorAll('[data-ai]').forEach(button=>button.onclick=async()=>{
      const snapshot=JSON.stringify(read()),labels={correct:'Correcting the question with AI…',regenerate:'Regenerating the question with AI…',replace_from_paper:'Creating a new question from the paper syllabus and conditions…'};form.querySelectorAll('[data-ai]').forEach(b=>b.disabled=true);status.classList.add('busy');status.textContent=labels[button.dataset.ai]||'AI is preparing a suggestion…';status.scrollIntoView({block:'nearest'});
      ...
      }catch(error){status.textContent=error.message;}finally{status.classList.remove('busy');form.querySelectorAll('[data-ai]').forEach(b=>b.disabled=false);}
    });
