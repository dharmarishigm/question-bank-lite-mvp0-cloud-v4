(() => {
  ...
  fetch('/api/auth/config',{cache:'no-store'}).then(config=>{authConfig=config;
    ...
    if(config.client_id){
      ...
      script.onload=()=>{google.accounts.id.initialize({client_id:config.client_id,callback:async result=>{try{await json('/api/auth/google',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({credential:result.credential})});complete();}catch(error){byId('login-message').textContent=error.message;}}});google.accounts.id.renderButton(byId('google-signin'),{theme:'outline',size:'large',width:280});};
