...
def init_platform():
    ...
    from engagement_schema import SCHEMA as ENGAGEMENT_SCHEMA
    with closing(db()) as conn:
        conn.executescript(ENGAGEMENT_SCHEMA); conn.commit()
    from grand_test_schema import SCHEMA as GRAND_TEST_SCHEMA
    with closing(db()) as conn:
        conn.executescript(GRAND_TEST_SCHEMA); conn.commit()
    ...
    from student_content_guard import SCHEMA as CONTENT_SCHEMA
    with closing(db()) as conn:
        conn.executescript(CONTENT_SCHEMA); conn.commit()
...
async def auth_google(request:Request,response:Response):
    ...
    if selected=='ADMIN':
        with closing(db()) as conn:
            existing=conn.execute("SELECT role FROM users WHERE lower(trim(email))=?",(email,)).fetchone()
        if email not in _admins() and (not existing or existing['role']!='ADMIN'):raise HTTPException(403,'Administrator access has not been granted to this Google account')
    if selected=='FLAG':
...
async def admin_login(request:Request,response:Response):
    ...
    with closing(db()) as conn:
        ...
        # Development-only convenience: the configured local administrator is
        # provisioned/promoted on first password login. Production still
        # requires an explicitly created ACTIVE administrator account.
        if os.getenv('APP_ENV') in {'development','test'} and email==expected_email:
            if user:
                conn.execute("UPDATE users SET role='ADMIN',status='ACTIVE',email_verified=1,updated_at=? WHERE id=?",(now,user['id']))
            else:
...
def exam_program_context(conn,rows):
    """Use persisted program-paper links, never guess an exam's program from its title."""
