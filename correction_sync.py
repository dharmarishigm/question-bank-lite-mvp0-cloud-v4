...
def propagate(conn,old,updated,user_id,plans):
    ...
    for row in conn.execute("SELECT id FROM exams WHERE current_version_id IS NOT NULL AND status IN ('PUBLISHED','OPEN','CLOSED') ORDER BY id").fetchall():
        ...
        for q in questions:
            ...
            value={k:updated[k] for k in ['statement','options','answer','solution']}
...
def annotate(conn,questions,originals,include_answers=False):
    """Return corrections as extra information, never overwrite scored snapshots."""
    by_id={q['id']:q for q in originals};ids=list(by_id)
    if not ids:return questions
    ...
    current={}
    for row in rows:
        q=dict(row);q['options']=json.loads(q['options'] or '[]');q['statement']=statement_with_question_figures(q['statement'],json.loads(q.pop('visual_assets') or '[]'))
        previous=by_id[q['id']]
        if any(q.get(k)!=previous.get(k) for k in ['statement','options','answer','solution']):current[q['id']]=q
    for q in questions:
        latest=current.get(q['id'])
        if latest:q['correction']={k:latest[k] for k in (['statement','options','answer','solution'] if include_answers else ['statement','options'])}
    return questions
