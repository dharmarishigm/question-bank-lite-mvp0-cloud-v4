"""Bounded staging-only 100-student exam test; credentials stay in process memory."""
...
assert os.environ['CLOUD_RUN_JOB']=='qb-security-staging-load100'
RUN='load100-'+str(int(time.time()))
...
with closing(db()) as conn:
    ...
    conn.commit()
print(json.dumps({'event':'seeded','run':RUN,'exam_id':exam,'students':100,'duration_minutes':10}),flush=True)
...
async def call(client,kind,method,path,body=None):
    ...
    try:
        ...
        if r.status_code!=200:raise RuntimeError(f'{kind}: HTTP {r.status_code}')
        return r.json()
...
async def student(i,user):
    ...
    async with httpx.AsyncClient(headers=headers,timeout=15,follow_redirects=False) as c:
        counted=False
        try:
...
try:
    ...
    report={'event':'summary','run':RUN,'exam_id':exam,'peak_active':peak,'completed':completed,'correctly_scored':scored,'status_counts':dict(statuses),'errors':failures,'latency_ms':{k:{'count':len(v),'p95':percentile(v,.95),'p99':percentile(v,.99),'max':round(max(v),1)} for k,v in timings.items()},'synthetic_sessions_revoked':True,'passed':completed==100 and scored==100 and not failures}
