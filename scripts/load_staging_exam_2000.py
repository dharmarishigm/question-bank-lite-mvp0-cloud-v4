...
assert os.environ['CLOUD_RUN_JOB']=='qb-security-staging-load2000'
RUN='load2000-'+str(int(time.time()))
...
with closing(db()) as conn:
    ...
    conn.commit()
print(json.dumps({'event':'seeded','run':RUN,'exam_id':exam,'students':2000,'duration_minutes':10}),flush=True)
