"""Bounded staging-only 2000-student exam test; credentials stay in process memory."""
...
assert os.environ['CLOUD_RUN_JOB']=='qb-security-staging-load2000-3h'
RUN='load2000-3h-'+str(int(time.time()))
...
with closing(db()) as conn:
    ...
    exam=conn.execute("INSERT INTO exams(name,description,duration_minutes,total_marks,status,allow_self_registration,created_by,created_at,updated_at) VALUES(?,?,180,180,'OPEN',0,?,?,?)",(RUN,'Synthetic capacity test only',owner,now,now)).lastrowid
    for i in range(180):
    ...
    for i in range(2000):
        ...
        conn.execute('INSERT INTO app_sessions(token_hash,user_id,csrf_token,expires_at,created_at) VALUES(?,?,?,?,?)',(digest,uid,csrf,now+43200,now))
        users.append({'id':uid,'token':token,'csrf':csrf,'hash':digest})
    conn.commit()
print(json.dumps({'event':'seeded','run':RUN,'exam_id':exam,'students':2000,'duration_minutes':180}),flush=True)
...
async def student(i,user):
    ...
    try:await asyncio.wait_for(stop.wait(),timeout=i*.09)
...
try:
    ...
    with closing(db()) as conn:
        scored=conn.execute("SELECT COUNT(*) n FROM exam_sessions WHERE exam_id=? AND status='SUBMITTED' AND score=180 AND correct_count=180",(exam,)).fetchone()['n']
