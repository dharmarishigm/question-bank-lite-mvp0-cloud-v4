...
def main():
    ...
    parser.add_argument('--scenario',choices=['linear','jee-chemistry','mental-ability-text','mental-ability-visual'],default='linear')
    ...
    def probe(kind):
        ...
        try:
            if kind=='generation':
                ...
                if args.scenario.startswith('mental-ability'):
                    visual=args.scenario.endswith('visual')
                    request=GenerationRequest(exam_name='Synthetic mental ability validation',subject='Mental ability and EVS',level='Class 5',difficulty='medium',question_type='mcq_single',count=1 if visual else 3,syllabus='Number patterns, classification, everyday environmental science. This is a synthetic scope, not a verified official syllabus.',generation_prompt=('Create one original non-verbal visual pattern question with a simple question figure and four distinct diagram choices. Provide a complete visual_spec using supported primitives, answer label and a concise worked solution.' if visual else 'Create three original age-appropriate questions: one number sequence, one classification problem, and one everyday environmental science question. Each must have four distinct non-empty text options, one answer label and a concise worked solution.'))
                ...
                if args.scenario.startswith('mental-ability'):
                    checks={'question_count':len(batch.questions)==request.count,'four_options':all(len(item.options)==4 for item in batch.questions),'solutions':all(item.solution.strip() for item in batch.questions),'matching_answer_labels':all(item.answer.strip().upper() in [option.label for option in item.options] for item in batch.questions)}
                    if args.scenario.endswith('visual'):checks['visual_spec']=all(item.visual_required and item.visual_spec is not None for item in batch.questions)
        ...
        try:
            with patch('prompt_registry.resolve_active_prompt',side_effect=lambda key:resolve_active_prompt(key,conn=conn)):
                with ThreadPoolExecutor(max_workers=2) as pool:
                    results=list(pool.map(probe,['generation','guidance'] if args.scenario=='linear' else ['generation']))
        finally:
            conn.close()
