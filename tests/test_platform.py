...
class PlatformSecurityTests(unittest.TestCase):
    ...
    def test_marketing_home_seo_and_workspace_boundary(self):
        ...
        self.assertIn('<loc>https://meritiqra.com/</loc>',sitemap.text)
        self.assertNotIn('/admin',sitemap.text)
        self.assertNotIn('/student',sitemap.text)
        self.assertNotIn('/app',sitemap.text)
        workspace=self.client.get('/app')
        self.assertEqual(workspace.status_code,200)
        self.assertEqual(workspace.headers.get('cache-control'),'no-store')
        workspace_html=workspace.text
