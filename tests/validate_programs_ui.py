...
async def main():
    async with async_playwright() as pw:
        ...
        await page.get_by_role('button',name='Back to Programs',exact=True).click()
        ...
        card = page.locator('[data-program-list] article').filter(has_text='QA Navodaya sample').last
        await card.get_by_role('button',name='Open program',exact=True).click()
        await page.get_by_role('button',name='Create a program exam',exact=True).wait_for()
        assert 'section=overview' in page.url
        await page.get_by_role('button',name='Create exam',exact=True).click()
        assert 'section=create-exam' in page.url
        await page.go_back();await page.get_by_role('button',name='Create a program exam',exact=True).wait_for()
        assert 'section=overview' in page.url
        await page.go_forward();await page.get_by_text('Create an exam for QA Navodaya sample',exact=True).wait_for()
        await page.get_by_role('button',name='Overview',exact=True).click()
        await page.locator('[data-program-content]').get_by_role('button',name='Manage published exams',exact=True).click()
        await page.locator('[data-return-program]').wait_for()
        await page.get_by_role('button',name='← Back to QA Navodaya sample',exact=True).click()
        await page.get_by_role('button',name='Papers',exact=True).wait_for()
        ...
        await page.reload();await page.locator('[data-program-tab="QUESTION_GENERATOR"][aria-current="true"]').wait_for(state='attached')
        await page.get_by_text('Advanced program workspace',exact=True).click()
