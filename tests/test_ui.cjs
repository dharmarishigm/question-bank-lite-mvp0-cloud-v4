...
assert.ok(run("toHtml('[open](javascript:alert%281%29)')").includes('open'));
assert.equal(run("toHtml('[open](javascript:alert%281%29)')").includes('javascript:'), false);
...
assert.ok(run("toHtml('$x^2$')").includes('$x^2$'));
