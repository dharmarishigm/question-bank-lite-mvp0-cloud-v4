...
for i in range(5):
 e=post(a,'/api/admin/exams',{'name':f'Science Practice {i+1}','status':'DRAFT','question_ids':qs,'duration_minutes':30,'allow_self_registration':True})
 for state in ('PUBLISHED','OPEN'):
  r=a.put(f'/api/admin/exams/{e["id"]}/state',json={'status':state},headers={'X-CSRF-Token':a.cookies['qb_csrf']});r.raise_for_status()
 post(s,f'/api/exams/{e["id"]}/enroll');sid=post(s,f'/api/exams/{e["id"]}/sessions',{'consent':True,'consent_client':{'user_agent':'browser-validation'}})['session_id']
...
with sync_playwright() as p:
 ...
 for role,client,views in [('student',s,['dashboard','available-exams','my-exams','my-results','student-analytics','my-profile']),('admin',a,['dashboard','questions','ai-generate','upload-crop','exam','admin-exams','admin-results','student-analytics'])]:
  ...
  for width,height in [(1440,900),(820,1180),(390,844)]:
   ...
   for view in views:
    ...
    if view=='student-analytics':
     page.wait_for_selector('#analytics-content:not([hidden])')
     assert page.locator('#analytics-kpis .exam-analytics-card').count()>=6
  ...
  if role=='student':
   ...
   assert page.locator('#analytics-kpis .exam-analytics-card').count()>=6
   assert page.locator('#analytics-trend svg').count()==1
   page.set_viewport_size({'width':390,'height':844});page.screenshot(path=str(out/'performance-mobile.png'))
  context.close()
 browser.close()
