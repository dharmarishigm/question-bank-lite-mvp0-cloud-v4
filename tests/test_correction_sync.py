...
def test_admin_takes_exam_and_does_not_enter_student_ranking(clients):
    ...
    e=admin.post('/api/admin/exams',json={'name':'Admin participation','status':'OPEN','allow_self_registration':False,'question_ids':[q['id']]}).json()
    ...
    start=admin.post(f'/api/student/exams/{e["id"]}/start',json={});assert start.status_code==200,start.text
...
def test_corrected_versions_future_attempts_and_historical_notices(clients):
    ...
    e=admin.post('/api/admin/exams',json={'name':'Versioned exam','status':'OPEN','question_ids':[q['id']]}).json()
    student.post(f'/api/exams/{e["id"]}/enroll')
    sid=student.post(f'/api/student/exams/{e["id"]}/start',json={}).json()['session_id']
    ...
    fresh=admin.post(f'/api/student/exams/{e["id"]}/start',json={}).json()['session_id']
...
def test_correction_version_does_not_publish_unrelated_bank_edits(clients):
    ...
    exam=admin.post('/api/admin/exams',json={'name':'Selective correction','status':'OPEN','question_ids':[a['id'],b['id']]}).json()
    with app.connect() as conn:
